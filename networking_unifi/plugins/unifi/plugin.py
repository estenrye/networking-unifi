...
# Import Ml2Plugin before l3_db to fix https://github.com/projectunifi/unifi/issues/8494
from neutron.plugins.ml2.plugin import Ml2Plugin
from neutron.db import l3_db
from neutron.db.models import l3

from networking_unifi.compat import cfg
from networking_unifi.compat import constants
from networking_unifi.compat import log


LOG = log.getLogger(__name__)


class UnifiPlugin(Ml2Plugin, l3_db.L3_NAT_db_mixin):
    ...
    def __init__(self):
        ...
        super(UnifiPlugin, self).__init__()

    # Intercept floating IP associates/disassociates so we can trigger an
    # appropriate endpoint update.
    def _update_floatingip(self, context, id, floatingip):
        LOG.info("UnifiPlugin _update_floatingip: %s", floatingip)
        old_floatingip, new_floatingip = super(
            UnifiPlugin, self)._update_floatingip(context, id, floatingip)

        LOG.info("UnifiPlugin new_floatingip=%s", new_floatingip)
        ...
        LOG.info("UnifiPlugin old_floatingip=%s", old_floatingip)
    ...
    def create_floatingip(self, context, floatingip,
        ...
        LOG.info("UnifiPlugin create_floatingip: %s", floatingip)
        new_floatingip = super(UnifiPlugin, self).create_floatingip(
            context,
            floatingip,
